...
def run_aliyun_ocr(image: np.ndarray, cfg: dict[str, Any]) -> list[OcrChar]:
    log = get_logger("ocr")

    if not cfg.get("access_key_id") or not cfg.get("access_key_secret"):
        log.warning("ocr(aliyun): access_key 未配置，跳过 OCR")
        return []
    ...
    try:
        ...
        log.debug(f"ocr(aliyun): {len(chars)} chars recognized")
        return chars
    except Exception as e:
        log.warning(f"ocr(aliyun) 调用失败 ({type(e).__name__}: {e})")
        return []
...
def _parse_response(response: Any) -> list[OcrChar]:
    ...
    try:
        ...
        get_logger("ocr").debug(f"ocr(aliyun) parse error: {e}")
